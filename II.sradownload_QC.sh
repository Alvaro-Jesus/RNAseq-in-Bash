...
echo "-----------------------------------------------"
echo "SRA download and QC by Alvaro Huamani Ortiz"
echo "Version 2.0, November 18th 2025, from prefetch to MultiQC ~~discoverthevibes~~"
echo "-----------------------------------------------"
...
if [ -d "$filename" ]; then
	echo "Folder '$filename' already exists. Be careful idiot"
    ...
    # Step 3.1 - Prefetch the SRA data
        ...
        for i in {1..3}; do
            if prefetch "${sra}"; then
                ...
                echo "Attempt $i failed for ${sra}, retrying in 30 seconds..."
                sleep 30
            fi
        done

        if [ "$success" = false ]; then
            echo "Error: prefetch failed for ${sra} after 3 attempts, no way!"
            exit 1  # stop script 
        fi

    # Step 3.2 - Convert SRA to FASTQ using fasterq-dump
    if ! fasterq-dump --mem 12G \
                      --threads 12 \
                      --skip-technical \
